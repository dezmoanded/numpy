...
(let* ((A (mini-array:asarray '((1 2 3) (4 5 6)))))
  (let* ((s0 (mini-array:sum-axis A 0))
         (l0 (mini-array:to-list s0)))
    (assert-equal (length l0) 3 "axis=0 length: ")
    (assert-list-approx l0 '(5 7 9) "axis=0 sums: "))
  (let* ((s1 (mini-array:sum-axis A 1))
         (l1 (mini-array:to-list s1)))
    (assert-equal (length l1) 2 "axis=1 length: ")
    (assert-list-approx l1 '(6 15) "axis=1 sums: ")))

;; Error paths: 0D, 1D, invalid axes
(expect-error (lambda () (mini-array:sum-axis (mini-array:asarray 7) 0))
              "0D should error: ")
(expect-error (lambda () (mini-array:sum-axis (mini-array:asarray '(1 2 3)) 0))
              "1D should error: ")

(let ((A (mini-array:asarray '((1 2) (3 4)))))
  (expect-error (lambda () (mini-array:sum-axis A -1)) "axis=-1 should error: ")
  (expect-error (lambda () (mini-array:sum-axis A 2))  "axis=2 should error: "))

;; Optional: alias check
(let* ((A (mini-array:asarray '((1 2 3) (4 5 6))))
       (s0 (mini-array:sum_axis A 0))
       (l0 (mini-array:to-list s0)))
  (assert-equal (length l0) 3 "alias sum_axis axis=0 length: ")
  (assert-list-approx l0 '(5 7 9) "alias sum_axis axis=0: "))

(format t "OK: reduce tests passed~%")
(sb-ext:exit :code 0)
